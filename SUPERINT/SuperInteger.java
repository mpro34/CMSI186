...
public class SuperInteger { 
	...
	public String toBinary () {
		String st = this.superint;
		long superINT = Long.parseLong(st);
		long[] superlist = new long[st.length()+2];
		String myString = "";
		System.out.println("H E R E");
		
		for (int i=st.length()+1; i>0; i--) {
			superlist[i] = superINT%(long)2;
			System.out.println("Ar: " + Arrays.toString(superlist));
			System.out.println("sI: "+superINT);
			superINT = (long)Math.ceil(superINT/2);
			System.out.println("sI: "+superINT);
			System.out.println("ARRAY: "+superINT%(long)2);
			if ((superINT) < (long)1) { break; }
		}
		System.out.println("G O O D");
		for (int i=0; i<superlist.length; i++) {
			String addString = Integer.toString((int)superlist[i]);
